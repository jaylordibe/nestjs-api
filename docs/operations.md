...
| Soft-delete **hard purge** | ❌ Operator-owned | Soft-deleted users are retained indefinitely. Nothing purges them. The shape to copy is `auth.refresh-token-retention.v1` — a handler beside the data it sweeps, plus one entry in `RECURRING_SCHEDULES` (`src/common/queue/recurring-schedule-registry.ts`). |
| Refresh-token purge | ✅ Implemented **and scheduled** | `auth.refresh-token-retention.v1`, a BullMQ recurring job at `0 0 * * *` UTC, run by the worker runtime. Expired rows carry a user id and a device fingerprint, so they are deleted rather than kept. Idempotent — the delete is bounded by `expiresAt < now`. |
...
| Membership **history** | ⚙️ Scaffolded | The row is **current state, not a ledger** — a re-join overwrites `joinedAt`, `endedAt`, `status`, and `roleId` in place. Tenure history lives in `audit_logs` (`business_membership.*`, `business_invitation.accepted`) and is **best-effort**: `AuditService.record` swallows its own failures and runs after the membership transaction commits, so an overwritten tenure can lose its event. Useful for investigation, **not a system of record**. `audit_logs` also has no retention policy — see the row above. Guaranteed tenure needs an append-only period table written in the same transaction; this template ships none. |
