...
- **Auth** — `POST /auth/register`, `POST /auth/login`, `GET /auth/me`, `POST /auth/logout` (revokes the current token's `jti` via Redis blocklist), `POST /auth/logout-all` (bumps `passwordChangedAt` so every outstanding token for the user is rejected). All /me/logout/logout-all routes use `JwtAuthGuard`. Login + register override the global throttle with stricter per-route limits (10/min and 5/min) to slow brute-force attempts.
