...
src/
  ...
  common/
    ...
    enums/             # RoleScope, PermissionOwnership, SeededRoleName, Gender, OtpPurpose, AppPlatform,
                       # DeviceType, DeviceOs, BusinessMembershipStatus, BusinessInvitationStatus, TimeScope
                       # (+ enum-option-meta.ts — the shape the enums module projects to clients)
  ...
  modules/
    authorization/     # @Global: AbilityFactory, PermissionLoaderService (grants cache),
                       ...
                       # PermissionCheckService (NARROW escape hatch — see below),
                       # guards/PermissionsGuard (global APP_GUARD, fails closed),
                       # PermissionCatalogIntegrityService + RouteAuthorizationAuditService
                       # + AuthorizationDataIntegrityService (boot gates)
    ...
    auth/ users/ roles/ businesses/ (memberships, invitations) audit-logs/ app-versions/ device-tokens/ health/ public/
...
- **Tenant isolation lives in the QUERY, not the guard.** A guard runs before the row is loaded and CASL ignores conditions on a subject-*type* check, so it can only prove a rule *exists*. Scope every read through `AbilityScopedQueryService` (`buildWhere` / `buildWhereOrEmpty` / `buildRecordWhere`). **Never import `@casl/prisma` elsewhere — ESLint blocks it**: Prisma silently drops an empty `OR: []` nested inside `AND`, so the obvious merge returns *every row* to a caller with no rules. **404 when the caller cannot read the record** (a 403 would confirm it exists); **403 when they can read it but may not act on it**. Full contract: `src/common/authorization/README.md`.
  - **`PermissionCheckService` is the one narrow exception, and it is not an alternative.** `assertCan(ability, action, subjectType, record)` checks an **already-loaded** record, for the single case a `where` clause cannot express: derived ownership needing a join ("I may edit this booking because I own the business that owns it") — load the parent, then assert against it. Everything else stays in the query, because a filtered read never loads the cross-tenant row and yields the 404 the rule above requires. Its other method, `createAbilityForUser(userId)`, exists for work with **no HTTP request** (queue handlers, `prisma/scripts/`); inside a request use `@CurrentAbility()`, which the guard has already built — re-deriving it costs a Redis round trip for nothing. Reaching for either one to avoid scoping a list query is the failure mode this split exists to prevent.
...
- `.claude/settings.json` (committed) — the permissions floor, and **the only enforcement layer that exists**. A plugin cannot ship permission rules, and as of `engineering-framework` 1.0.0 the plugin ships no command-gating hook either: its single hook is `SessionStart` → `scripts/session-charter.sh`, which loads the charter text and gates nothing. Nothing else backstops this file, so a rule that fails to match is a hole, not a redundancy. It is also **not a sandbox** — a shell can always express an operation the parser does not model. For a real boundary use OS sandboxing or a container.
  ...
  - **`allow` and `ask` rules use the `verb:*` prefix form; `deny` uses `verb *` plus an exact-match twin for any command whose bare invocation is itself the dangerous one.** `Bash(yarn build *)` requires a space *and* an argument after `build`, so it never matches bare `yarn build` — an allow tier written that way prompts on every clean invocation, and an `ask` rule written that way lets the bare command fall through to a broader `allow`. The same gap in a `deny` is far worse: `Bash(docker compose down *)` alone leaves bare `docker compose down` unmatched. That is why the deny tier pairs `yarn prisma:migrate *` with `yarn prisma:migrate` (and the same for `prisma:deploy`, `prisma:reset`, `prisma:seed`), and `docker compose down *` with `docker compose down` — in both spellings, since the legacy hyphenated `docker-compose down` is a different binary the spaced pattern never sees.
  - **Six rules from the framework's `allow` floor must stay absent; they crept back in once, and re-adding them is a regression.** `docker compose down:*` (this repo *denies* teardown — and because the prefix form matches the bare command that the `docker compose down *` deny once missed, the allow entry actively **granted** the exact operation the file claims to deny), `docker exec:*` (a container shell — `docker exec … psql` reaches the developer's local data, and the `Bash(psql *)` ask rule never fires because the command is `docker exec`), and `yarn|npm|pnpm|bun run:*` (`yarn run prisma:migrate` escapes the `yarn prisma:migrate *` deny entirely — a spelling of a human-owned operation with nothing behind the rule to catch it). Use `yarn <script>` directly; the scripts this repository actually runs are allowed by name.
  - **Still allowed, and known: `docker compose exec:*` / `docker compose run:*`.** Either can reach a `psql` shell in a database container without passing the `psql` ask rule. They stay allowed because routine container work needs them; confirm the target is the test stack (5434/6380) before running a statement that writes.
- `.claude/engineering-framework.json` (committed) — this repository's policy, and **nothing else**: `frameworkVersion`, canonical `commands`, and `risk.highRiskPaths` + `risk.notes`. The 1.0.0 schema is `additionalProperties: false` and has **no permission or enforcement keys at all** — `protectedCommands`, `protectedPaths`, `useDefaultProtectedPaths` and the `policy` switches were removed with the guard hook, so adding any of them back makes the file fail validation rather than protecting anything. The compose-teardown and `psql` protections live in `settings.json` alone; the reason never to edit an applied migration lives in `risk.notes`, where the design and review gates read it.
  - **`highRiskPaths` patterns keep their leading `*/`** — a bare `prisma/migrations/**` matches nothing, because patterns are tested against the absolute path.
  - **`risk` is advisory to an agent, not a block.** It raises the ceremony a change gets (full plan, threat model, negative tests, multi-lens review); it cannot stop an edit. Migrations, deployments, Git writes, and pull requests stay human-owned because the gates hand off, and because `settings.json` denies the commands.
