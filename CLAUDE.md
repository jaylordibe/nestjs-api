...
- `.claude/engineering-framework.json` (committed) — this repository's policy: canonical commands, `risk.highRiskPaths`, the `*/prisma/migrations/*` protected path, and the `protectedCommands` the framework's generic tables cannot know about (`docker compose down` in both spellings, denied; `docker exec … psql`, held at `ask`). The leading `*/` in a path pattern is load-bearing — a bare `prisma/migrations/**` matches nothing, because patterns are tested against the absolute path.
  - **The file carries no `policy` block, and adding one is a regression.** Every switch is already at the default this repository wants — in particular `humanOwnedDependencyInstall`, which defaults to **false**: adding and removing packages is ordinary engineering here, and `useDefaultProtectedPaths` covers the lockfile, so the diff is visible at review regardless. An absent key and a key written out at its default behave identically, so the shorter file is the one that stays accurate. Migrations, deployments, Git writes, and pull requests all stay human-owned; publication, release, force pushes, and credential reads are denied regardless of this setting.
