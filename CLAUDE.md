...
- **Verify before declaring done.** `yarn build` + `yarn lint` + the **affected** e2e spec(s) must pass on every change. **Evidence uses `yarn lint`, never `yarn lint:fix`** — `eslint --fix` exits 0 after silently rewriting whatever it repaired, so the fixing form cannot fail and proves nothing. `lint` is the gate and never edits your files; `lint:fix` is the local fixer you run *while* writing. Prettier is enforced inside `lint` via the `prettier/prettier` rule, so there is deliberately no separate `format` script. run the **full** `yarn test:e2e` only when a module is complete or the user asks. The e2e suite runs **in parallel** (`maxWorkers: 50%`): `globalSetup` migrates one template database and clones it per worker, and each worker gets its own Redis logical database (`test/setup/worker-isolation.ts`) — so specs must never assume exclusive access to anything outside their own database. Type-check and a passing suite verify correctness, but don't claim a UI/feature works without actually exercising it.
